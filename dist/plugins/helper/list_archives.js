...
const postsCache = new hexo_util_1.Cache();
function listArchivesHelper(options = {}) {
    ...
    const posts = config.relative_link ? postsCache.apply(`date-${order}`, () => this.site.posts.sort('date', order)) : this.site.posts.sort('date', order);
    if (!posts.length)
        return result;
    const data = [];
    let length = 0;
